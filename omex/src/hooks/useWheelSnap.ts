...
export function useWheelSnap({ container, timeout = 600 }: Options) {
  ...
  useEffect(() => {
    ...
    function doSnap(direction: number) {
      ...
      const viewportTop = root instanceof Document ? window.scrollY : (root as HTMLElement).scrollTop;
      let currentIndex = 0;
      for (let i = 0; i < sections.length; i++) {
        const rect = sections[i].getBoundingClientRect();
        const top = root instanceof Document ? rect.top + window.scrollY : rect.top + (root as HTMLElement).scrollTop;
        if (top <= viewportTop + 10) currentIndex = i;
      }

      let nextIndex = currentIndex;
      if (direction > 0) nextIndex = Math.min(sections.length - 1, currentIndex + 1);
      ...
      if (nextIndex !== currentIndex) {
        ...
        if (root instanceof Element) {
          ...
          (root as HTMLElement).scrollTo({ top: offsetTop, behavior: 'smooth' });
        } else {
    ...
    function onTouchEnd(e: TouchEvent) {
      ...
      const touchEndY = e.changedTouches && e.changedTouches.length ? e.changedTouches[0].clientY : null;
